	...
	if (!is_single()) {
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
            ...
            // 第一页
						...
						echo '<span class="page-index"><a href="'.esc_url(get_pagenum_link()).'" class="first" title="'.$first_page_text.'">'.$first_page_text.'</a></span>';
        ...
        echo '</span>';
          ...
          if(!empty($pages_text)) {
					...
					if ($end_page < $max_page) {
						...
						echo '<span class="page-last"><a href="'.esc_url(get_pagenum_link($max_page)).'" class="last" title="'.$last_page_text.'">'.$last_page_text.'</a></span>';
					}
					echo '</div>'.$after."\n";
          //yes this is the end
		}
...
function tie_pagenavi_init() {
	...
	$pagenavi_options['first_text'] = __('&laquo; 首页');
	$pagenavi_options['last_text'] = __('尾页 &raquo;');
	$pagenavi_options['next_text'] = __('&raquo;','tie');
	$pagenavi_options['prev_text'] = __('&lt;&lt;','tie');
	$pagenavi_options['dotright_text'] = '...';
	$pagenavi_options['dotleft_text'] = '...';
	
	//设置显示在分页数字当中显示多少个，3就是显示3个，比方说345然后是下一页
