...
function tie_get_pagenavi($query = false, $num = false , $before = '', $after = '') {
	...
	if (!is_single()) {
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			echo $before.'<div class="pagebar"><div class="pagination">'."\n";
            ...
            // dotleft_text
              ...
              // 
							...
							echo '<a href="'.esc_url(get_pagenum_link($i)).'" class="page page-item page-link st" title="'.$page_text.'">'.$page_text.'</a>';
					...
					for($i = $start_page; $i  <= $end_page; $i++) {						
						if($i == $paged) {
							...
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
							echo '<a href="'.esc_url(get_pagenum_link($i)).'" class="page-item page-link st" title="'.$page_text.'">'.$page_text.'</a>';
						}
					}
          ...
          if(!empty($pages_text)) {
						echo '<a class="page-item page-link" href="javascript:;">'.$pages_text.'</a>';
					}
          echo '</div></div>'.$after."\n";
					}
